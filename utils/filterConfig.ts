...
export const FILTER_CONFIG = {
  ...
  'World Cup': {
    ...
    subFilters: [
      ...
      { name: 'Morocco', order: 13 },
      { name: 'Norway', order: 14 },
    ],
    detector: (jersey: any) => {
      const name = jersey.name.toUpperCase();
      return /2026|WORLD|CUP|ARGENTINA|BRAZIL|FRANCE|GERMANY|SPAIN|ENGLAND|ITALY|PORTUGAL|MEXICO|CROATIA|JAPAN|JAMAICA|MOROCCO|NORWAY/.test(name);
    },
  },
  ...
  'Clubs': {
    ...
    subFilters: [
      { name: 'All', order: 0 },
      { name: 'Real Madrid', order: 1 },
      { name: 'Barcelona', order: 2 },
      { name: 'Manchester', order: 3 },
      { name: 'Liverpool', order: 4 },
      { name: 'Chelsea', order: 5 },
      { name: 'Juventus', order: 6 },
      { name: 'Inter Milan', order: 7 },
      { name: 'AC Milan', order: 8 },
      { name: 'PSG', order: 9 },
      { name: 'Monaco', order: 10 },
      { name: 'Inter Miami', order: 11 },
      { name: 'Alnassr', order: 12 },
      { name: 'Santos', order: 13 },
      { name: 'Sao Paulo', order: 14 },
    ],
    detector: (jersey: any) => {
      const name = jersey.name.toUpperCase();
      const clubs = [
        'REAL MADRID', 'BARCELONA', 'MANCHESTER', 'LIVERPOOL', 'CHELSEA', 'JUVENTUS',
        'INTER MILAN', 'INTER', 'AC MILAN', 'PSG', 'MONACO', 'INTER MIAMI', 'ALNASSR',
        'SANTOS', 'SAO PAULO',
      ];
...
export const SUBFILTER_MATCHERS: Record<string, Record<string, (name: string) => boolean>> = {
  ...
  'Clubs': {
    ...
    'Santos': (name) => /SANTOS/.test(name.toUpperCase()),
